...
const CancelOrderModal = ({ setCancel, cancel, refetch }) => {
    ...
    return (
        <div>
            ...
            <label htmlFor="cancel-order-modal" className="modal cursor-pointer">
                <label className="modal-box relative" htmlFor="cancel-order-modal">
                    <label htmlFor="cancel-order-modal" className="btn btn-sm btn-circle absolute right-2 top-2">✕</label>
                    <h3 className="text-xl text-red-400 font-bold">Are you sure you want to delete "{cancel.name}" ?</h3>
